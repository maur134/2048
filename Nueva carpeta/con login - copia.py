...
class Login:
    ...
    def sin_login(self):
        ...
        self.root.deiconify()
        game = Game2028(self.root)
        game.hide_login_register_buttons()
            ...
            for usuario in usuarios:
                if usuario["user_name"] == name_user:
                    ...
                    if usuario["password"] == password:
                        ...
                        self.login_frame.destroy()
                        self.root.deiconify()
                        juego = Game2028(self.root, user_name=name_user)
...
class Register:
    ...
    def register_logic(self, event=None):
        ...
        if user_name != "" and password != "" and password == password_confirmed:
            datos = {"user_name": user_name, "password": password, "best_score": 0}  # Inicializar con el mejor puntaje como 0
            with open("users.json", "r") as file:
                usuarios = json.load(file)
...
class Menu_principal:
    ...
    def main_menu (self):
        self.men = ctk.CTkFrame(master = self.menu_prin, width = 1042, height = self.y)
        self.men.place(x = self.x - 992, rely = 0.5, anchor = tk.CENTER)

        stats = ctk.CTkFrame(master = self.menu_prin, width = 428, height = self.y)
        stats.place(x = self.x-428)
        a = self.y - 958
        b = self.y - a
        user_info = ctk.CTkFrame(master = self.menu_prin, width = 394, height = b, fg_color = "#4DBA8B", corner_radius = 50)
        user_info.place(x = -43, y = a)
     

class Game2028:
    def __init__(self, ventana, user_name=None):
